...
We first decide on the website to scrap, in our case a shop in NZ. We will limit ourselves to the **bathroom** items. 
...
We want to web scrap pages from  the website `r mainurl` to get some prices of products. It can be good to have a look and see whether there is a `robot.tx` file with some restrictions. 

> Here there is no restriction and no *robot.txt* 

Here we restrict ourselves to a subpage on `r subpage` , so the page of interest is finally `r url` 
...
The page itself is a complex object in Html, and is not easy to read for a human, but machines can do that and identify elements such as **nodes** and from there links to subpages "<a href "

Here is an excerpt of the page downloaded: 

```{r}
# Print the first 100 characters of the page content
cat(substr(as.character(page), 1, 100))
```
...
We also need to pay attention to empty fields 
